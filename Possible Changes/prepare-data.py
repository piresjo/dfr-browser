...
def convert_state(statef, twout, dtout, n):
    with gzip.open(statef, 'rb') as f:
        f.readline()
        alpha = map(float, f.readline().strip().split(" ")[2:])
        beta = f.readline().strip().split(" ")[2]
        print("beta value, not saved in a file: " + beta)
    ...
    write_tw(alpha,
            [transform_topic_weights(tw[t], vocab, n) for t in xrange(K)],
            twout)
